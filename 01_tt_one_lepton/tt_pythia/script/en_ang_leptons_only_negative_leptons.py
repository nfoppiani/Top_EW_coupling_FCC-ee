...
h_EL = TH2F("only Electrons","only Electrons",16,0.,1.,16,-1.,1.)
h_MU = TH2F("only - Muons","only - Muons",16,0.,1.,16,-1.,1.)
h_ELplusMU = TH2F("only negative leptons","only negative leptons",16,0.,1.,16,-1.,1.)
...
c_EL = TCanvas("Only Electrons","Only Electrons",800,800)
c_EL.cd()
h_EL.Draw("surf")

c_MU = TCanvas("Only - Muons","Only - Muons",800,800)
c_MU.cd()
h_MU.Draw("surf")

c_ELplusMU = TCanvas("Only negative leptons","only negative leptons",800,800)
c_ELplusMU.cd()
...
mc_e_mu_energy_histo = TFile("./../plot/negative_lepton_distribution_histo.root","CREATE")
h_EL.Write()
h_MU.Write()
h_ELplusMU.Write()
...
mc_e_mu_energy_histo = TFile("./../plot/negative_lepton_distribution_canvas.root","CREATE")
c_EL.Write()
c_MU.Write()
c_ELplusMU.Write()
