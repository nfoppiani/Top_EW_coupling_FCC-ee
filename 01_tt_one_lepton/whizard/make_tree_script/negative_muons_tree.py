...
chain.Add("/afs/cern.ch/user/t/tpajero/work/public/whizard_negMuTau_yyxylv/000/yy*.root")
chain.Add("/afs/cern.ch/user/t/tpajero/work/public/whizard_negMuTau_yyxylv/001/yy*.root")

# creates the new file and the tree that it will contain
savingFile = TFile("../tree/negative_muons_tree.root", "CREATE")
negative_muonsTree = TTree('negative_muons_tree', 'reducedInformationTree')
...
negative_muonsTree.Branch('mcTyp', mcTyp, 'mcTyp/D')
negative_muonsTree.Branch('mcEne', mcEne, 'mcEne/D')
negative_muonsTree.Branch('mcRedEne', mcRedEne, 'mcRedEne/D')
negative_muonsTree.Branch('mcMox', mcMox, 'mcMox/D')
negative_muonsTree.Branch('mcMoy', mcMoy, 'mcMoy/D')
negative_muonsTree.Branch('mcMoz', mcMoz, 'mcMoz/D')
negative_muonsTree.Branch('mcCosTheta', mcCosTheta, 'mcCosTheta/D')
negative_muonsTree.Branch('mcInvMas', mcInvMas, 'mcInvMas/D')
...
for event in chain:
	...
	if chain.mcpdg[10]==13:
		...
		negative_muonsTree.Fill()        

savingFile.cd()
negative_muonsTree.Write()         # writes the tree in the savingFile
savingFile.Close()
