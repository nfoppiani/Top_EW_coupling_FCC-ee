...
savingFile = TFile("../tree/negative_tau_tree.root", "CREATE")
negative_tauTree = TTree('negative_tau_tree', 'reducedInformationTree')
...
negative_tauTree.Branch('mcTyp', mcTyp, 'mcTyp/D')
negative_tauTree.Branch('mcEne', mcEne, 'mcEne/D')
negative_tauTree.Branch('mcRedEne', mcRedEne, 'mcRedEne/D')
negative_tauTree.Branch('mcMox', mcMox, 'mcMox/D')
negative_tauTree.Branch('mcMoy', mcMoy, 'mcMoy/D')
negative_tauTree.Branch('mcMoz', mcMoz, 'mcMoz/D')
negative_tauTree.Branch('mcCosTheta', mcCosTheta, 'mcCosTheta/D')
negative_tauTree.Branch('mcInvMas', mcInvMas, 'mcInvMas/D')

for event in chain:
	# identifies a particle of the event, varies from 1 to numberOfParticles
	
	# loops on the particles contained in every event
	if chain.mcpdg[10]==15:
		...
		negative_tauTree.Fill()        

savingFile.cd()
negative_tauTree.Write()         # writes the tree in the savingFile
savingFile.Close()
