...
file = TFile('./yyveyx_o_1.root',"READ")
tree = file.Get("MyLCTuple")

j = 0               # j is the event progressive number

for event in tree:
    w = 0           # 1 if there is a W decaying in a positron in the current event
    t = 0           # 1 if there is a top in the current event
    notBoth = 0     # 1 if there is a top but not a W decaying in a positron in the current event, or vice-versa
    ...
    print 'END EVENT ', j, ' ######## ', '1 if there is a positron-decaying w:', w, ' ##### ', '1 if there is a top-antiTop:', t
    print
    j = j+1

print 'There are ', notBoth, ' events with only one between (a w which decays in a positron) and (a top or an antitop).'