...
eosDir1=/store/cmst3/user/nfoppian/ttbar365Wizard/yyxyev/
eosDir2=/store/cmst3/user/tpajero/ttbar365Whizard/yyxyev/
...
for ((job=1;job<3;job++));
  do
  echo "JOB "$job
  name="yyxyev_wizard_o_"${job}
  slcioFilename="yyxyev_o_rec_5248_"${job}".slcio"
  ntupleFilename="yyxyev_o_"${job}".root"
